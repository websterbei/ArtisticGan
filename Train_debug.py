...
def get_ops(geneator, discriminator):
    ...
    D_optim = tf.train.AdamOptimizer(learning_rate=0.0002).minimize(D_loss, var_list=discriminator.vars())
    G_optim = tf.train.AdamOptimizer(learning_rate=0.0002).minimize(G_loss, var_list=generator.vars())
